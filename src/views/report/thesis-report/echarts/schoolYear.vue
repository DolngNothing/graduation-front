...
<template>
  <div>
    <!-- 内容 -->
    <div id="thesisSchoolYear" style="width: 600px;height: 500px;"></div>
  </div>
</template>
      ...
      if (res.data.code === 1) {
        ...
        for (j = 0, len = this.schoolList.length; j < len; j++) {
          ...
          this.tempList = this.thesisList.filter((item) => {
            return item.student.school.name.includes(school.name) && item.passDate.includes(year)
          })
          console.log('ok')
        ...
        const myChart = this.$echarts.init(document.getElementById('thesisSchoolYear'))
