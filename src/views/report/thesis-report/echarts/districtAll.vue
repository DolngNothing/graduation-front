...
<template>
  <div>
    <!-- 内容 -->
    <div id="thesisDisAll" style="width: 600px;height: 500px;"></div>
  </div>
</template>
      ...
      this.$axios.get('test').then(res => {
        if (res.data.code === 1) {
          ...
          for (j = 0, len = this.district.length; j < len; j++) {
            ...
            this.opinion.push(this.temp.name)
          ...
          const myChart = this.$echarts.init(document.getElementById('thesisDisAll'))
