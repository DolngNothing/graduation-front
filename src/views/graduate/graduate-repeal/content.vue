      ...
      <el-col v-for="(item) in graduateList" :key="item.id" style="width:370px;">
        <el-card class="box-card" shadow="hover">
          <div slot="header" class="clearfix">
            <span class="spanTitle2">{{ item.major.name }}</span>
            <el-button :disabled="!item.certified" style="float: right; padding:10px 10px 10px 10px " type="danger" icon="el-icon-delete" @click.native.prevent="select(item)">选择删除</el-button>
          </div>
